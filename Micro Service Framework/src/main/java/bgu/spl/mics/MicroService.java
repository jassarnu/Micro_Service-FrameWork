...
public abstract class MicroService implements Runnable {
    ...
    private ConcurrentHashMap<Class<? extends Event<?>>,Callback> EventsCallbacks =new ConcurrentHashMap<>();
    private ConcurrentHashMap<Class<?extends Broadcast>,Callback> BrodcastsCallbacks=new ConcurrentHashMap<>();
    private CountDownLatch latch_finish;
    ...
    public void setLatch_finish(CountDownLatch latch_finish) {
        this.latch_finish = latch_finish;
    }
    ...
    protected final <T, E extends Event<T>> void subscribeEvent(Class<E> type, Callback<E> callback) {
        synchronized (EventsCallbacks) {
            EventsCallbacks.putIfAbsent(type, callback);
    ...
    protected final <B extends Broadcast> void subscribeBroadcast(Class<B> type, Callback<B> callback) {
        synchronized (BrodcastsCallbacks) {
            BrodcastsCallbacks.putIfAbsent(type, callback);
            ...
            try {
                ...
                if(msg!=null) {
                    if (msg instanceof Broadcast) {
                        Callback tmp = BrodcastsCallbacks.get(msg.getClass());
                        tmp.call(msg);
                    } else {
                        Callback tmp = EventsCallbacks.get(msg.getClass());
                        tmp.call(msg);
                    }
                }
            } catch (Exception e) {
